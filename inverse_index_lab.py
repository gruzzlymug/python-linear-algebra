...
def makeInverseIndex(strlist):
    ...
    """
    invIdx = {}
    for (idxText,text) in enumerate(strlist):
        for (idxWord,word) in enumerate(text.split()):
            if word not in invIdx:
                invIdx[word] = set()
            invIdx[word].add(idxText)
    return invIdx
...
def orSearch(inverseIndex, query):
    ...
    for word in query:
        if word in inverseIndex:
            docIds = docIds.union(inverseIndex[word]) 
    return docIds
...
def andSearch(inverseIndex, query):
    ...
    {0, 4}
    """
    docIds = inverseIndex.get(query.pop(), set())
    for word in query:
       docIds = docIds & inverseIndex.get(word, set())
    return docIds
